...
configure_pycortex=true
for arg in "$@"; do
    case "$arg" in
        --skip-pycortex-config)
            configure_pycortex=false
            ;;
        *)
            echo "Unknown argument: $arg"
            echo "Usage: bash bin/get_data.sh [--skip-pycortex-config]"
            exit 1
            ;;
    esac
done
...
# WebSSL MAE
if [ ! -f "data/external/webssl_mae300m_full2b_224.pth" ]; then
    wget -O data/external/webssl_mae300m_full2b_224.pth https://dl.fbaipublicfiles.com/webssl/webssl_mae300m_full2b_224.pth
fi
...
if [ ! -d "data/external/THINGS" ]; then
    wget -O data/external/THINGS.zip https://osf.io/rdxy2/download
    unzip -P things4all data/external/THINGS.zip -d data/external/
    mv data/external/object_images data/external/THINGS
    rm data/external/THINGS.zip
fi

# thingsplus
if [ ! -d "data/external/THINGSplus" ]; then
    wget -O data/external/THINGSplus.zip https://osf.io/wb36u/download
    unzip -P things4all data/external/THINGSplus.zip -d data/external/
    mv data/external/object_images_CC0 data/external/THINGSplus
    rm data/external/THINGSplus.zip
fi
...
# things fmri data
uv run bin/get_brain.py
if [ "$configure_pycortex" = true ]; then
    uv run bin/fix_pycortex.py
else
    echo "Skipping pycortex user config update."
fi
...
## ImageNet ##
# this is quite wasteful as we only use a handful of the images, but i don't know if there is a better way
