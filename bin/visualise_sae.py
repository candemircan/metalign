...
def create_image_grid(image_paths, title, values=None):
    if not image_paths: return f"<h3>{title}</h3><p>No images found</p>"
    images_html = ""
    for i, img_path in enumerate(image_paths):
        img_base64 = image_to_base64(img_path)
        img_name = Path(img_path).name
        val_html = f"<br><small><b>{values[i]:.4f}</b></small>" if values is not None else ""
        images_html += f"""
        <div style="display: inline-block; margin: 5px; text-align: center;">
            <img src="{img_base64}" style="max-width: 150px; max-height: 150px;"/>
            <br><small>{img_name}</small>{val_html}
        </div>
        """
    return f"""
    <div>
...
def create_app(image_paths, features, col_indices):
    ...
    def server(input, output, session):
        ...
        def feature_stats():
            ...
            orig_dim = col_indices[input.feature_dim()]
            return f"Feature {input.feature_dim()} (original SAE dim {orig_dim}) of {features.shape[1]} total features. {data['n_zeros']} images ({fraction_zeros:.1%}) have value 0. {data['n_above_threshold']} images ({fraction_above_threshold:.1%}) above threshold {input.threshold()}"
        ...
        def image_display():
            ...
            nonzero_vals = [data['values'][i] for i in selected_nonzero]
            ...
            html_content = create_image_grid(nonzero_paths, title1, nonzero_vals) + "<hr style='margin: 30px 0;'/>" + create_image_grid(zero_paths, title2)
...
@call_parse
def main(
    image_dir: str = "data/external/coco/train2017",
    feature_path: str = "data/sae/coco_train_sae-top_k-64-cls_only-layer_11-hook_resid_post.h5",
    min_nonzero: int = 100,
    port: int = 8000,
):
    image_paths = [str(x) for x in sorted(Path(image_dir).glob("*.jpg"))]
    features, col_indices = h5_to_np(features_path=feature_path, min_nonzero=min_nonzero, return_col_indices=True)

    if len(image_paths) != features.shape[0]:
        print(f"Warning: {len(image_paths)} images but {features.shape[0]} feature rows")

    app = create_app(image_paths, features, col_indices)
    app.run(port=port) 