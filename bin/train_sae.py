...
from metarep.data import SAEFunctionLearning
...
@call_parse
def main(
    train_backbone: str = "coco_dinov2_vitb14_reg",  # backbone from which the training representations are extracted. the name must match data/backbone_reps/{train_backbone}.npz
    test_backbone: str = "things_dinov2_vitb14_reg",  # backbone from which the test representations are extracted. the name must match data/backbone_reps/{test_backbone}.npz
    train_sae_features: str = "coco_sae-top_k-64-cls_only-layer_11-hook_resid_post",  # SAE features for training data
    test_sae_features: str = "things_sae-top_k-64-cls_only-layer_11-hook_resid_post",  # SAE features for test data
    ...
    min_nonzero: int = 100,  # minimum number of non-zero activations per column to keep it in the final array for SAE features
):
    """
    train a meta-learning transformer model over SAE function learning tasks with separate train and test datasets
    """
    args = locals()
    if config_file:
    ...
    # Load train data
    train_representations = np.load(f"data/backbone_reps/{args["train_backbone"]}.npz")
    ...
    # Load test data
    test_representations = np.load(f"data/backbone_reps/{args["test_backbone"]}.npz")
    ...
    if args["num_components"] is not None:
        ...
        train_data.X = torch.from_numpy(pca.fit_transform(train_data.X)).to(torch.float32)
        train_data.feature_dim = args["num_components"]
        
        test_data.X = torch.from_numpy(pca.transform(test_data.X)).to(torch.float32)
        test_data.feature_dim = args["num_components"]

    config = TransformerConfig(
        input_size=train_data.feature_dim,
    ...
    # Use all dimensions from train data for training
    train_dims = torch.tensor(list(range(train_data.Y.shape[1])), device=device)
    # Use all dimensions from test data for evaluation  
    test_dims = torch.tensor(list(range(test_data.Y.shape[1])), device=device)
    ...
    for training_step in pbar:
        ...
        for i in range(args["batch_size"]):
            ...
            X_episode, Y_episode = train_data.sample_episode(dim, args["sequence_length"], args["fixed_label"], args["weighted"])
        ...
        if (training_step + 1) % args["eval_interval_steps"] == 0:
            with torch.no_grad():
                ...
                for i in range(args["num_eval_episodes"]):
                    dim = test_dims[i % len(test_dims)] # cycle through test_dims
                    X_episode, Y_episode = test_data.sample_episode(dim, args["sequence_length"], args["fixed_label"], args["weighted"])
