...
Replicates eval_thingso1o.R analysis in Python.
"""
import json
...
def prepare_data(df, is_main, df_main=None):
    ...
    # Participant indices (thingso1o uses subject_id)
    participants = df['subject_id'].unique()
    p2idx = {p: i for i, p in enumerate(participants)}
    p_idx = df['subject_id'].map(p2idx).values
...
def main(
    ...
    print(f"Running eval_thingso1o_map.py for {experiment_name} {backbone_name}")

    stats_file = Path(f"data/evals/thingso1o/{experiment_name}_{backbone_name}_stats.csv")
    ...
    df_main = None
    if not is_main:
        main_file = Path(f"data/evals/thingso1o/MAIN_{backbone_name}_stats.csv")
    ...
    output_file = Path(f"data/evals/thingso1o/{experiment_name}_{backbone_name}_analysis_map.json")
