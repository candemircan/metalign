...
if length(ARGS) < 2
    error("Usage: julia eval_thingso1o.jl <experiment_name> <backbone_name>")
end

experiment_name = ARGS[1]
backbone_name = ARGS[2]
csv_file = "data/evals/thingso1o/$(experiment_name)_$(backbone_name)_stats.csv"
...
if !is_main
    main_csv_file = "data/evals/thingso1o/MAIN_$(backbone_name)_stats.csv"
...
if is_main
    # Model 0: Base only
    formula_0 = @formula(is_choice ~ base_sim + (1 + base_sim | subject_id))
    ...
    formula_1 = @formula(is_choice ~ base_sim + ablation_sim + (1 + base_sim + ablation_sim | subject_id))
    ...
    formula_2 = @formula(is_choice ~ base_sim + ablation_sim + main_sim + (1 + base_sim + ablation_sim + main_sim | subject_id))
...
output_file = "data/evals/thingso1o/$(experiment_name)_$(backbone_name)_analysis.json"
