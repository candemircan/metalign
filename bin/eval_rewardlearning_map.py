...
Replicates eval_rewardlearning.R analysis in Python.
"""
import json
...
def prepare_data(df, is_main, df_main=None):
    ...
    # Calculate diffs
    df['base_diff'] = df['base_val_1'] - df['base_val_0']
    df['metalign_diff'] = df['metalign_val_1'] - df['metalign_val_0']

    if not is_main and df_main is not None:
        df['main_diff'] = df_main['metalign_val_1'].values - df_main['metalign_val_0'].values

    # Z-score predictors
    df['base_diff'] = z_score(df['base_diff'])
    df['metalign_diff'] = z_score(df['metalign_diff'])
    if 'main_diff' in df.columns:
        df['main_diff'] = z_score(df['main_diff'])
...
def fit_map(df, p_idx, n_p, pred_names, n_steps=5000, lr=0.01):
    ...
    y = jnp.array(df['human_choice'].values)
    n_obs = len(y)
...
def main(
    ...
    print(f"Running eval_rewardlearning_map.py for {experiment_name} {backbone_name}")

    stats_file = Path(f"data/evals/rewardlearning/{experiment_name}_{backbone_name}_stats.csv")
    ...
    df_main = None
    if not is_main:
        main_file = Path(f"data/evals/rewardlearning/MAIN_{backbone_name}_stats.csv")
    ...
    if is_main:
        ...
        stats0 = fit_map(df, p_idx, n_p, ['base_diff'])
        ...
        stats1 = fit_map(df, p_idx, n_p, ['base_diff', 'metalign_diff'])
        ...
        lrt = calculate_lrt(stats0, stats1)
        print(f"Log10 BF (M1/M0): {log10_bf:.2f}, LRT p-value: {lrt['p_value']:.3e}")

        results['model_0'] = stats0
        results['model_1'] = stats1
        results['comparison_0_vs_1'] = {'log10_bayes_factor': log10_bf, 'lrt': lrt}
    else:
        ...
        stats2 = fit_map(df, p_idx, n_p, ['base_diff', 'metalign_diff', 'main_diff'])
    ...
    output_file = Path(f"data/evals/rewardlearning/{experiment_name}_{backbone_name}_analysis_map.json")
