...
if (length(args) < 2) {
    stop("Usage: Rscript eval_rewardlearning.R <experiment_name> <backbone_name>")
}

experiment_name <- args[1]
backbone_name <- args[2]

cat(sprintf("Running eval_rewardlearning.R for %s %s\n", experiment_name, backbone_name))

stats_file <- sprintf("data/evals/rewardlearning/%s_%s_stats.csv", experiment_name, backbone_name)
...
if (!is_main) {
    main_stats_file <- sprintf("data/evals/rewardlearning/MAIN_%s_stats.csv", backbone_name)
    if (!file.exists(main_stats_file)) stop("Main model stats file not found for ablation")
    df_main <- read.csv(main_stats_file)

    df$main_diff <- df_main$metalign_val_1 - df_main$metalign_val_0
}
...
if ("main_diff" %in% names(df)) df$main_diff <- z_score(df$main_diff)
...
save_results(results, sprintf("data/evals/rewardlearning/%s_%s_analysis_r.json", experiment_name, backbone_name))
