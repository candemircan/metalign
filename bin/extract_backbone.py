...
@call_parse
def main(
    dataset: Param(help="Dataset to use", choices=['things', 'thingsplus', 'coco', 'levels']),
    ...
    elif repo_id.startswith("webssl/"):
        weights_path = Path(f"data/external/{repo_id.split('/')[-1]}.pth")
        vit = _create_vision_transformer('vit_large_patch16_224.mae', patch_size=16, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4.0)
        vit.load_state_dict(torch.load(weights_path, map_location=device, weights_only=True))
        vit = vit.to(device).eval()

        class WebSSLModel(torch.nn.Module):
            def __init__(self, vit): super().__init__(); self.vit = vit
            def forward(self, x): return self.vit.forward_features(x)[:, 1:].mean(dim=1)

        model = WebSSLModel(vit)
    ...
    if dataset == "coco": splits = ['train', 'eval']
    else: splits = ['']
