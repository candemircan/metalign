...
__all__ = ["ImageDataset", "Things", "Coco", "h5_to_np", "FunctionDataset", "prepare_things_spose", "load_backbone", "Levels", "prepare_levels", "DATASET_MAKERS"]
...
def h5_to_np(features_path: Path,
                min_nonzero: int = 1,
                return_col_indices: bool = False,
    ...
    Args:
        features_path: Path to the h5 file with the features.
        min_nonzero: Minimum number of non-zero activations per column to keep it in the final array. The default is such that any column that is all 0s is removed.
        return_col_indices: If True, return (activations, col_indices) tuple.
    """
    ...
    if activations.shape[1] > 0:
        ...
        keep_mask = non_zero_counts >= min_nonzero
        col_indices = np.where(keep_mask)[0]
        activations = activations[:, keep_mask]
    else:
        col_indices = np.array([], dtype=np.intp)

    if return_col_indices: return activations, col_indices
    return activations
...
class FunctionDataset(Dataset):
    ...
    def __init__(self, inputs: np.ndarray, features_path: Path,
                 seq_len: int = 120, min_nonzero: int = 120, train_dims: list = None, epoch_size: int = None, episodes_per_dim: int = None):
        ...
        self.episodes_per_dim = episodes_per_dim
    ...
    def __len__(self):
        if self.episodes_per_dim is not None: return len(self.train_dims) * self.episodes_per_dim
        if self.epoch_size is not None: return self.epoch_size
        return 2**31 - 1 # effectively infinite for training (when epoch is not set)

    def __getitem__(self, idx):
        if self.episodes_per_dim is not None: dim = self.train_dims[idx // self.episodes_per_dim]
        else: dim = self.train_dims[torch.randint(0, len(self.train_dims), (1,)).item()]
    ...
    def _sample_episode(self, dim: int):
        ...
        raw_values = self.Y[indices, dim]

        if self.is_sparse:
            # sparse: positive = non-zero, magnitude is the value itself
            Y_episode = (raw_values != 0).float()
            Y_magnitude = raw_values.clone()
        else:
            # dense: positive = above median, magnitude is signed distance from median
            Y_episode = (raw_values > median_val).float()
            Y_magnitude = raw_values - median_val

        return X_episode, Y_episode, Y_magnitude


class ImageFunctionDataset(Dataset):
    """Per-image dataset for StaticLinear: returns labels for all SAE dims per image (no episode structure)."""
    def __init__(self, inputs:np.ndarray, Y:np.ndarray, is_sparse:bool,
                 medians:torch.Tensor|None=None, mag_stds:torch.Tensor|None=None):
        self.X = torch.tensor(inputs, dtype=torch.float32)
        self.Y = torch.from_numpy(Y)
        self.is_sparse = is_sparse
        if not is_sparse: self.medians = medians if medians is not None else torch.median(self.Y, dim=0).values

        # per-function magnitude std for normalization (computed from training, reused for eval)
        if mag_stds is not None:
            self.mag_stds = mag_stds
        elif is_sparse:
            stds = []
            for d in range(self.Y.shape[1]):
                vals = self.Y[:, d]
                nz = vals[vals != 0]
                stds.append(nz.std().item() if len(nz) > 1 else 1.0)
            self.mag_stds = torch.tensor(stds, dtype=torch.float32)
        else:
            self.mag_stds = (self.Y - self.medians.unsqueeze(0)).std(dim=0)
        self.mag_stds = self.mag_stds.clamp(min=1e-8)
    ...
    def __getitem__(self, idx):
        y = self.Y[idx]
        if self.is_sparse:
            return self.X[idx], (y != 0).float(), y / self.mag_stds
        return self.X[idx], (y > self.medians).float(), (y - self.medians) / self.mag_stds


def prepare_things_spose(
    representations: np.ndarray,
    data_root: Path = Path("data/external"),
    ...
    Args:
        representations: Directly loaded from `load_backbone(h5_path)`.
        data_root: The root directory of the THINGS dataset, which contains the images and the unique_id.txt file.
        return_tensors: The type of tensors to return, can be "pt" for PyTorch tensors or "np" for NumPy arrays.
    """
...
def prepare_levels(
    ...
    Args:
        representations: Directly loaded backbone representations.
        data_root: The root directory containing levels.pkl.
        return_tensors: The type of tensors to return, can be "pt" for PyTorch tensors or "np" for NumPy arrays.
