__all__ = ["fix_state_dict", "set_pycortex_filestore_path", "calc_cka", "calc_class_separation", "calc_r2"] 
...
def calc_r2(preds: torch.Tensor, targets: torch.Tensor) -> float:
    """
    calculate r2 for a batch.
    """
    if targets.numel() < 2: return 0.0 # Not enough data for variance
    
    ss_res = torch.sum((targets - preds) ** 2)
    ss_tot = torch.sum((targets - targets.mean()) ** 2)
    
    if ss_tot < 1e-6: return 0.0 # Constant target, R2 undefined/0
        
    return 1.0 - (ss_res / ss_tot).item()

def calc_cka(X:torch.Tensor, Y:torch.Tensor):
    """
    Calculate centered kernel alignment (CKA) between two batched tensors.

    Args:
        X (torch.Tensor): First tensor (batch, observations, features).
        Y (torch.Tensor): Second tensor (batch, observations, features).
    """
...
def set_pycortex_filestore_path(new_path:str):
    """
    Programmatically sets the Pycortex filestore location in the options.cfg file.
    This is not a great solution if you use pycortex for other project, as I believe it overrides the global config. But it has been a pain, and this is what we have for now.
...
def calc_class_separation(X:torch.Tensor, y:torch.Tensor):
    """
